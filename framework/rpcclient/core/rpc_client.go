...
var inputMutex sync.Mutex  //控制收包后内存拷贝走之后再执行下一个
var outputMutex sync.Mutex //重连操作锁控制，避免重复重连
...
type RpcClient struct {
	...
	//Net层ip和端口
	remoteIp   string //Net层ip
	remotePort int    //Net层端口
...
func (rpcclient *RpcClient) getSequence() uint64 {
	...
	rpcclient.seq++
	//plog.Debug("Cur seq=", rpcclient.seq)
	return rpcclient.seq
}

//从请求表中剔除某个序列号
func (rpcclient *RpcClient) deleteReqRecord(seq uint64) error {
	...
	if !isfind {
		plog.Info("deleteReqRecord cann't find reqrecord seq:", seq)
		err := errors.New("deleteReqRecord cann't find reqrecord seq")
		return err
	}
...
func (rpcclient *RpcClient) addReqRecord(seq uint64, rpcReqRecd *rpcRequestRecord) {
	...
	if isfind {
		plog.Info("addReqRecord find a repeated reqrecord seq:", seq)
		old.close()
	}
	//plog.Debug("addReqRecord, seq = ", seq)
...
func (rpcclient *RpcClient) printRecMap() {
	...
	for k, v := range rpcclient.reqMap {
		plog.Info("k:", k, "rec:status ", v.status, "seq: ", v.seq)
	}
}
...
func (rpcclient *RpcClient) setSvrType(serv_type int64) {
	_ = rpcclient.initflag[0] //初始化标志，主要预防业务使用的人没有调用newRpcClient， TODO怎么样实现防止没有人调用newRpcClient
...
//设置Net层ip、port
func (rpcclient *RpcClient) setRemoteAddress(remoteip string, remoteport int) {
	rpcclient.remoteIp = remoteip
	rpcclient.remotePort = remoteport
}
...
func (rpcclient *RpcClient) clientInput() {
	...
	for !rpcclient.isexit {
		inputMutex.Lock() //接收内存区锁
		//获取整包长度datalen
		//plog.Debug("rpcclient.isexit = ", rpcclient.isexit)
		if err := rpcclient.net.ReadAtLeast(protolen, 4); err != nil {
			if err.Code() == 1 {
				//连接失败的时候（比如：断线重连时） 重新读
				plog.Fatal("readAtLeast err and the err code is:", err.Code())
				inputMutex.Unlock() //接收内存区解锁
				time.Sleep(10 * time.Millisecond)
				continue
			} else {
				plog.Fatal("io exception  ", err)
				rpcclient.close()   //整个服务进程都要结束
				inputMutex.Unlock() //接收内存区解锁
				return
			}
		}
		...
		if err := rpcclient.net.ReadAtLeast(data, int(datalen-4)); err != nil {
			if err.Code() == 1 {
				plog.Fatal("continue and the err code is:", err.Code())
				inputMutex.Unlock() //接收内存区解锁
				time.Sleep(10 * time.Millisecond)
				//连接失败的时候（比如：断线重连时） 重新读
				continue
...
func (tcpclient *RpcClient) processPacket(vbgfmsg WholeMsg) {
	defer plog.CatchPanic()
	var bgfmsg *WholeMsg
	bgfmsg = &vbgfmsg
	inputMutex.Unlock() //接收内存区解锁
	plog.Debug("server get a Packet cmd: ", fmt.Sprintf("%x", bgfmsg.hd.Cmd))
	...
	case srpc.RPC_NOTIFY_PACKET:
		//return ProcessRpcNotify(data, len)	//处理收到的点对组的无需响应的包
...
func (rpcclient *RpcClient) processServiceDelete(bgfmsg *WholeMsg) {
	...
	if err != nil {
		plog.Fatal("ServiceDelete body err!")
		return
	}
	if 8 != *resp.RetCode {
		plog.Fatal("Register Dis failed ret", *resp.RetCode)
	}
	plog.Info("Service need deleted!!! returncode is................. ", *resp.RetCode)
	rpcclient.close() //关闭
...
func (rpcclient *RpcClient) processRegisterFuncsResp(bgfmsg *WholeMsg) {
	...
	err := resp.Unmarshal(bgfmsg.bd)
	if err != nil {
		plog.Fatal("processRegisterFuncsResp Unmarshal err!")
		return
	}
	if 0 != *resp.Result {
		plog.Fatal("Register Dis failed ret", resp.Result)
	}
	//plog.Info("processRegisterFuncsResp succeed!!! returncode is................. ", *resp.Result)
...
func (rpcclient *RpcClient) processKeepAliveResp(bgfmsg *WholeMsg) {
	...
	//	rpcclient.last_heart_time = time.Now()
	plog.Info("receive heart beat response")
}
...
func (rpcclient *RpcClient) TimeNotify() {
	...
	case DIS_CLOSE: //需要重连
		plog.Warn("TimeNotify start reconnect dispatch")
		rpcclient.reconnect()
...
func (rpcclient *RpcClient) sendPacket(req *RpcRecvReq, data []byte, rpccall *RpcCall) (err error) {
	...
	if err != nil {
		plog.Fatal("SendPacket err1", err)
	}
	//启动追踪
	var rpcReqRecd rpcRequestRecord
...
func (rpcclient *RpcClient) clientOutput(bfgmsg *WholeMsg, rpcReqRecd *rpcRequestRecord) {
	...
	byteOrderOp.Util_hton_int32(int32(datalen+4), lenbuf) //第一个4字节存入
	...
	if perr != nil {
		//plog.Info("NET ERR", getNetHeadString(rpcReqRecd.bghead))
		plog.Fatal("NET ERR,", perr.Code(), perr.Err(), getNetHeadString(rpcReqRecd.bghead))
		...
		if rpcReqRecd.rpchead != nil {
			plog.Fatal("NET ERR", GetRpcHeadString(rpcReqRecd.rpchead))
		}

		rpcReqRecd = nil //等待垃圾回收处理

		//设置状态，准备重连
		rpcclient.status = DIS_CLOSE
		//socket send失败时发起重连
		rpcclient.reconnect()
	}
}
...
//rpc端发起重连操作
//重连原因：net重启时，conn连接断开，rpc继续使用断开的socket去写数据就会导致panic！rpc端需要重新建立与Net的连接
func (tcpclient *RpcClient) reconnect() {
	...
	//断开之前的连接
	if tcpclient.status == DIS_CLOSE {

		tcpclient.net.Close()     //连接关闭的情况下才做重连操作
		tcpclient.timeroo.Close() //关闭定时器

		//重走注册的过程
		err := tcpclient.creatNet(tcpclient.remoteIp, tcpclient.remotePort)
		for err != nil {
			//rpc重连net层做200毫秒的时间间隔
			err = tcpclient.creatNet(tcpclient.remoteIp, tcpclient.remotePort)
			time.Sleep(200 * time.Millisecond)
		}
		tcpclient.status = DIS_CONNECTED //置为连接成功状态
		_ = <-tcpclient.regnotify        //等待注册成功的通知，收不到通知就一直堵塞在这里！
		tcpclient.registerFuncs()        //0x101服务注册成功之后，开始0x201方法注册

		//		tcpclient.net = new(rpcnet.TcpNet)
		//		tcpclient.net.SetReconTimes(2)
		//		err := tcpclient.net.Connect(tcpclient.localIp, tcpclient.localPort, tcpclient.remoteIp, tcpclient.remotePort)
		//		for err != nil {

		//			err = tcpclient.net.Connect(tcpclient.localIp, tcpclient.localPort, tcpclient.remoteIp, tcpclient.remotePort)
		//			time.Sleep(200 * time.Millisecond)
		//		}
		//tcpclient.status = DIS_REGISTE //TODO：重连的情况下是否需要注册？如果需要注册则次数还要改！
	}
}
...
func (rpcclient *RpcClient) creatNet(remoteip string, remoteport int) (err error) {
	...
	if rpcclient.svrtype < 1000 {
		plog.Warn("svrtype 必须大于1000")
		err = errors.New(" svrtype 必须大于1000")
		return
	}

	rpcclient.setRemoteAddress(remoteip, remoteport) //存储Net层ip和端口
...
func (rpcclient *RpcClient) launchRpcClient(remoteip string, remoteport int, rev Dispatch, szkclient *szk.SzkClient) (err error) {
	...
	go rpcclient.listenSysSignal()                                                                                                  //监听系统信号量，在收到kill信号量的时候做相应的处理
...
//信号监听（通过kill发送信号，接收后做处理）
func (rpcclient *RpcClient) listenSysSignal() {
	defer plog.CatchPanic()
	...
	go func() {
		...
		for {
			<-s
			plog.Debug("the framework get a sys signal")
			//do something here
			//信号控制配置文件重载（日志等级修改……）
			//测试信号控制重连的操作
			rpcclient.status = DIS_CLOSE //设置连接关闭
			rpcclient.reconnect()        //发起重连操作
		}
	}()
}
