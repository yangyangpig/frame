...
import (
	PGConfig "PGConfig/app/proto"
	"framework/rpcclient/core"
	"putil/log"
	"runtime"
	"strconv"
	"strings"
	"PGConfig/app/controllers"
	"github.com/astaxie/beego/config"
)

var (
	quitFlag chan bool          //退出标记
	configObj = new(controllers.ConfigController) //Server实例，为处理接收到的请求做准备
	svrtype  int64
	svrid    int64
	version  int32
	groupids []int64
...
func (dis *MyServiceDispatch) RpcRequest(req *rpcclient.RpcRecvReq, body []byte) {
	...
	case "Get":
		//接收参数并处理
		arithReq := PGConfig.ConfigRequest{}
		arithReq.Unmarshal(body)            //解参数的pb包
		arithResp := configObj.Dispatch(arithReq)
