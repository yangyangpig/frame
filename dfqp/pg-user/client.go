...
func main() {
	...
	//		5000表示5000毫秒后无响应就超时！
	response := client.SendAndRecvRespRpcMsg("pgUser.getUserInfo", req_bytes, 5000, 0)
	if response.ReturnCode != 0 {
		...
		arithResp := new(pgUser.GetUserInfoResponse)
	...
	req.Mid = 125673
	req.Nick = "hellogo222"
	req_bytes2, err := req.Marshal()
	if err != nil {
		fmt.Println(req_bytes2)
	}
	//		"arith"表示服务名
	//		Add表示调用的方法名
	//		req_bytes表示请求的参数（经过了protobuf的marshal后）
	//		5000表示5000毫秒后无响应就超时！
	response = client.SendAndRecvRespRpcMsg("pgUser.modifyUserInfo", req_bytes2, 5000, 0)
	if response.ReturnCode != 0 {
		//rpc返回结果异常
		//RPC_RESPONSE_COMPLET         = 0 //完成
		//	RPC_RESPONSE_TIMEOUT         = 1 //超时
