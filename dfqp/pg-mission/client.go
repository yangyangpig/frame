...
func main() {
	...
	//超载包后面加一个正常包
	request := new(pgMission.MissionListRequest)
	request.App = 1
	request.AreaId = 1
	request.Mid = 1
	request.ApiVer = "2001"
	request.HallVer = 2001
	...
	//		5000表示5000毫秒后无响应就超时！
	for i := 0; i < 1; i++ {
		response := client.SendAndRecvRespRpcMsg("PGMission.MissionList", req_bytes, 5000, 0)
		if response.ReturnCode != 0 {
			...
			arithResp := new(pgMission.MissionListResponse)
