...
var (
	...
	configObj = new(controller.ConfigController) //Server实例，为处理接收到的请求做准备
...
func (dis *MyServiceDispatch) RpcRequest(req *rpcclient.RpcRecvReq, body []byte) {
	...
	case "get":
		//接收参数并处理
		configReq := new(pgConfig.ConfigRequest)
		configReq.Unmarshal(body)            //解参数的pb包
		arithResp := configObj.Dispatch(configReq)
