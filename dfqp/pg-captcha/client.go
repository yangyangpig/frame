...
func main() {
	...
	//超载包后面加一个正常包
	request := new(pgCaptcha.GetCaptchaRequest)
	request.Phone = "18682307627"
	...
	//		5000表示5000毫秒后无响应就超时！
	response := client.SendAndRecvRespRpcMsg("pgCaptcha.getCaptcha", req_bytes, 5000, 0)
	if response.ReturnCode != 0 {
		...
		arithResp := new(pgCaptcha.GetCaptchaResponse)
		arithResp.Unmarshal(response.Body)
		plog.Debug("return value  = ", arithResp)
	}

	//request := new(pgCaptcha.GetVoiceCaptchaRequest)
	//request.Phone = "18682307627"
	//request.Type = 3
	//req_bytes, err := request.Marshal()
	//if err != nil {
	...
	////		req_bytes表示请求的参数（经过了protobuf的marshal后）
	////		5000表示5000毫秒后无响应就超时！
	//response := client.SendAndRecvRespRpcMsg("pgCaptcha.getVoiceCaptcha", req_bytes, 5000, 0)
	//if response.ReturnCode != 0 {
	//	//rpc返回结果异常
	//	//RPC_RESPONSE_COMPLET         = 0 //完成
	//	//	RPC_RESPONSE_TIMEOUT         = 1 //超时
	...
	//	plog.Debug("rpc return code = ", response.ReturnCode, " return err = ", response.Err)
	//} else {
	//	arithResp := new(pgCaptcha.GetVoiceCaptchaResponse)
	//	arithResp.Unmarshal(response.Body)
	//	plog.Debug("return value  = ", arithResp)
	//}
