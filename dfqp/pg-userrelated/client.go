...
func main() {
	...
	//超载包后面加一个正常包
	request := new(pgUserRelated.UserRelatedRequest)
	request.Mid = 143896
	request.S1 = 1
	...
	//		5000表示5000毫秒后无响应就超时！
	response := client.SendAndRecvRespRpcMsg("pgUserRelated.get", req_bytes, 5000, 0)
	if response.ReturnCode != 0 {
		...
		arithResp := new(pgUserRelated.UserRelatedResponse)
