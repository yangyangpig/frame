...
import (
	...
	"runtime"
	"strings"
	"dfqp/pg-userrelated/controller"
...
var (
	...
	userObj = new(controller.UserRelatedController) //Server实例，为处理接收到的请求做准备
...
func (dis *MyServiceDispatch) RpcRequest(req *rpcclient.RpcRecvReq, body []byte) {
	...
	case "get":
		//接收参数并处理
		userReq := new(pgUserRelated.UserRelatedRequest)
		userReq.Unmarshal(body)            //解参数的pb包
		arithResp := userObj.Dispatch(userReq)
